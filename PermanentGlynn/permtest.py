...
BASE, SYNTH, IMP, PLACE, ROUTE = (
    ...
    "implementation/CUSTOM33/xilinx_vivado/",
...
def get_tcl(isSynth=True):    
    ...
    dump_state PermanentTestKernel
    """
    return tcl
...
def dump_stats():
    ...
    result = os.system("vivado -mode batch -source dump.tcl")
    if result != 0: return
    ...
    for signed in (False, True):
        for size in range(2 if signed else 1, 256+1): #usefulSizes
            for strategy in range(3):
                ...
                retval = os.system("ant build -Dmanager=permanenttest.PermanentTestManager -Dtarget=DFE_SIM -Dmaxfile-name=PermanentTest_singleSIM -Dresult-dir=./builds/simulation -Dengine-params=\"frequency=100 inpX=" + str(-size[0] if signed else size[0]) + " inpY=" + str(-size[1] if signed else size[1]) + " strategy=" + str(strategy) + "\"")
                os.chdir(wd)
                if retval != 0: return
        ...
        for size in usefulSizes:
            for strategy in range(3):            
                ...
                while frequency != minfreq and frequency >= 6.25 and frequency <= 725.0:
                    wd = os.getcwd()
                    os.chdir(os.path.join(wd, "PermanentTestDFE"))
                    retval = os.system("ant build -Dmanager=permanenttest.PermanentTestManager -Dtarget=DFE -Dmaxfile-name=PermanentTest_singleDFE -Dresult-dir=./builds/bitstream -Dengine-params=\"frequency=" + str(frequency) + " inpX=" + str(-size[0] if signed else size[0]) + " inpY=" + str(-size[1] if signed else size[1]) + " strategy=" + str(strategy) + "\"")
                    os.chdir(wd)
                    if retval == 0:
                        minfreq = frequency
