...
#ifdef __DFE__
    if (lib == GlynnRepSingleDFE || lib == GlynnRepDualDFE || lib == GlynnRepMultiSingleDFE || lib == GlynnRepMultiDualDFE) {
        lock_lib();
        init_dfe_lib(DFE_MAIN, lib == GlynnRepDualDFE || lib == GlynnRepMultiDualDFE); 
        out_of_memory = false;
    }  
#endif
...
Complex16 calcPerm(matrix& mtx, PicState_int64& input_state, PicState_int64& output_state, int lib)
{
    Complex16 permanent;
    if (lib == GlynnRep) {
...
*/
        ...
        size_t idx_max_CPU = (lib == GlynnRepMultiSingleDFE || lib == GlynnRepMultiDualDFE || lib == GlynnRepSingleDFE || lib == GlynnRepDualDFE) ? colIndices.size()/6 : colIndices.size();
        ...
        if (lib == GlynnRepMultiSingleDFE || lib == GlynnRepMultiDualDFE || lib == GlynnRepSingleDFE || lib == GlynnRepDualDFE) tbb::parallel_invoke( 
            cpuFunc,
            [&](){
...
*/
                ...
                tbb::tick_count t0b = tbb::tick_count::now();////////////////////////// 
                if (lib == GlynnRepMultiSingleDFE || lib == GlynnRepMultiDualDFE) {
                    GlynnPermanentCalculatorRepeatedMultiInputBatch_DFE(column_reduced_matrix, batched_input_states, output_states_DFE, batched_perms, lib == GlynnRepMultiDualDFE);
                } else if (lib == GlynnRepSingleDFE || lib == GlynnRepDualDFE) {
                    GlynnPermanentCalculatorRepeatedInputBatch_DFE(column_reduced_matrix, batched_input_states, output_states_DFE, batched_perms, lib == GlynnRepDualDFE, false);
                }
                for (size_t idx=idx_max_CPU; idx<colIndices.size(); idx++) {
