...
def permanent_glynn(mat):
  ...
  n = len(mat)
  if n == 0: return 1
  return sum((-1 if (sum(x < 0 for x in delta) & 1) != 0 else 1) * functools.reduce(lambda x, y: x * y, (sum(delta[i] * mat[i][j] for i in range(n)) for j in range(n))) for delta in getDeltas(n)) >> n
